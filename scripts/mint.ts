...
function hexToUint64(hex: string): bigint {
  const b = BigInt(hex);
...
async function main() {
  const imgPath = process.env.PHOTO_PATH || process.argv[2]; // From env var or command line
  const tokenName = process.env.TOKEN_NAME || process.argv[3] || "My Photo #1";
  if (!imgPath) throw new Error("usage: node mint-photo.js <imagePath> [name] or set MINT_IMAGE_PATH env var");
  ...
  const ev = receipt?.logs?.find((l: any) => l.fragment?.name === "Minted");
